...
/**
 * Parsea una fecha/timestamp en múltiples formatos:
 * - "26-09-2025 10:13"  → formato Google Forms (DD-MM-YYYY HH:MM)
 * - "26/09/2025 10:13"  → variante con slash
 * - "26/09/2025"        → solo fecha con slash
 * - "26-09-2025"        → solo fecha con guion
 * - "2025-09-26"        → ISO estándar
 */
function parseFecha(str) {
  if (!str) return null;
  str = String(str).trim();

  // DD-MM-YYYY HH:MM o DD/MM/YYYY HH:MM (timestamp de Google Forms en español)
  const conHora = str.match(/^(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{4})\s+(\d{1,2}):(\d{2})/);
  if (conHora) {
    const [, d, m, y, h, min] = conHora;
    return new Date(`${y}-${m.padStart(2,'0')}-${d.padStart(2,'0')}T${h.padStart(2,'0')}:${min}:00`);
  }

  // DD/MM/YYYY o DD-MM-YYYY (solo fecha, sin hora)
  const soloFecha = str.match(/^(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{4})/);
  if (soloFecha) {
    const [, d, m, y] = soloFecha;
    return new Date(`${y}-${m.padStart(2,'0')}-${d.padStart(2,'0')}T00:00:00`);
  }

  // ISO u otro formato reconocido por Date
...
function prepararFilas(respRows, colTs, colBusNum, auditoriaDef) {
  ...
  for (let i = 1; i < respRows.length; i++) {
    ...
    // Validar timestamp (formato "DD-MM-YYYY HH:MM" de Google Forms en español)
    const tsRaw = row[colTs];
    if (!tsRaw) continue;
    const ts = parseFecha(String(tsRaw));
...
function calcularMetricas(latestPerBus, elementos, colIndexMap, colEstandar) {
  ...
  Object.entries(latestPerBus).forEach(([busNum, { row }]) => {
    ...
    // Incluye elementos del estándar del bus Y elementos compartidos (ej: "RED y Transantiago")
    const elementosAplicables = elementos.filter(el =>
      el.estandar === busEstandar || el.estandar.includes(busEstandar)
    );
...
// ============================================================
// SECCIÓN: HEATMAP BUSES × SUBGRUPO
// Retorna para cada bus auditado su % de cumplimiento por subgrupo.
// Usado por la pestaña "Tendencias".
// ============================================================
function buildHeatmapData(latestPerBus, elementos, colIndexMap, colEstandar, flotaMap) {
  // Subgrupos únicos en el orden en que aparecen en el catastro
  const subgruposOrden = [];
  const subgruposSet   = new Set();
  elementos.forEach(el => {
    if (!subgruposSet.has(el.subgrupo)) {
      subgruposSet.add(el.subgrupo);
      subgruposOrden.push(el.subgrupo);
    }
  });

  // Buses auditados ordenados numéricamente
  const buses = Object.keys(latestPerBus).sort((a, b) => {
  ...
  buses.forEach(busNum => {
    const { row } = latestPerBus[busNum];
    const busEstandar = String(row[colEstandar] || '').trim();
    valores[busNum] = {};

    subgruposOrden.forEach(sgNombre => {
      const elemsSubgrupo = elementos.filter(el =>
        el.subgrupo === sgNombre && !el.esAseo &&
        (el.estandar === busEstandar || el.estandar.includes(busEstandar))
      );

      if (elemsSubgrupo.length === 0) { valores[busNum][sgNombre] = null; return; }

      let positivos = 0, total = 0;
      elemsSubgrupo.forEach(el => {
        const mapKey = `${el.nombreColNorm}|${el.estandar}`;
        const colIdx = colIndexMap[mapKey];
        if (colIdx === undefined) return;
        const valorRaw = String(row[colIdx] || '').trim().toLowerCase();
        if (!valorRaw) return;
        total++;
        if (valorRaw === RESPUESTA_POSITIVA) positivos++;
      });

      valores[busNum][sgNombre] = total > 0 ? Math.round((positivos / total) * 100) : null;
    });

    // Total del bus: % sobre todos los elementos evaluables
    let totPos = 0, totEval = 0;
    elementos.filter(el =>
      !el.esAseo && (el.estandar === busEstandar || el.estandar.includes(busEstandar))
    ).forEach(el => {
      const mapKey = `${el.nombreColNorm}|${el.estandar}`;
      const colIdx = colIndexMap[mapKey];
      if (colIdx === undefined) return;
      ...
      if (!v) return;
      totEval++;
      if (v === RESPUESTA_POSITIVA) totPos++;
    });
    totales[busNum] = totEval > 0 ? Math.round((totPos / totEval) * 100) : null;
  });

  return { subgrupos: subgruposOrden, buses, valores, totales };
}
...
function buildBusesDetalle(latestPerBus, elementos, colIndexMap, colEstandar, flotaMap) {
  ...
  Object.entries(latestPerBus).forEach(([busNum, { row, ts }]) => {
    ...
    // Porcentaje total del bus (elementos con valor / positivos)
    const evaluables = items.filter(i => i.valor && !i.esAseo);
    const positivos  = evaluables.filter(i => norm(i.valor) === RESPUESTA_POSITIVA).length;
    const porcentajeTotal = evaluables.length > 0
      ? Math.round((positivos / evaluables.length) * 100)
      : null;
...
// ============================================================
function procesarDashboard({ respRows, datosRows, flotaRows, audRows, numeroAuditoria, colNames = {} }) {
  ...
  // Los nombres de columna pueden sobrescribirse por catastro via colNames.
  const headers     = respRows[0] || [];
  const colTs       = findCol(headers, colNames.timestamp || 'Marca temporal');
  const colBusNum   = findCol(headers, colNames.busNumero  || 'N° Interno del bus');
  const colEstandar = findCol(headers, colNames.estandar   || 'Señale el estándar del bus');
  ...
  // Solo se incluyen auditorías con número menor a la actual.
  // Las auditorías futuras a la seleccionada no se muestran.
  const historial = [];
  for (const aud of auditorias) {
    if (aud.numero === auditoriaDef?.numero) continue; // La actual se muestra aparte
    if (auditoriaDef && aud.numero > auditoriaDef.numero) continue; // Saltar auditorías futuras
    ...
    historial.push({
      ...
      // Porcentaje por grupo (nivel pestaña)
      porPestaña: Object.fromEntries(
        Object.entries(tf).map(([g, v]) => [g, v.porcentajeGeneral])
      ),
      // Porcentaje por subgrupo para gráficos de tendencia
      porSubgrupo: Object.fromEntries(
        Object.entries(tf).map(([g, v]) => [
          g,
          Object.fromEntries((v.subgrupos || []).map(sg => [sg.nombre, sg.porcentaje]))
        ])
      ),
    });
  }
  ...
  // --- 10b. Heatmap: porcentaje por bus × subgrupo ---
  const heatmapData = buildHeatmapData(latestPerBus, elementos, colIndexMap, colEstandar, flotaMap);
  ...
  return {
    ...
    dashboard: {
      ...
      // Buses de la flota actual que AÚN NO han sido auditados en este período
      busesPendientesList: Object.keys(flotaMap).filter(b => !latestPerBus[b]).sort(),
