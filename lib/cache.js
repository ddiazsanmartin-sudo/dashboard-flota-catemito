...
/**
 * Elimina todas las entradas cuya clave empieza por el prefijo dado.
 */
function invalidateByPrefix(prefix) {
  for (const key of store.keys()) {
    if (key.startsWith(prefix)) store.delete(key);
  }
  console.log(`[Cache] Entradas con prefijo "${prefix}" eliminadas.`);
}
...
module.exports = { get, set, invalidate, invalidateByPrefix, invalidateAll };
